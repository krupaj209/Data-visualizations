...
function WeeklyPattern() {
  ...
  const busiestIdxs = useMemo(
    () =>
      DATA.map((d, i) => (d.level === "busiest" ? i : -1)).filter(
        (i) => i >= 0,
      ),
    [],
  );
  const quietestIdx = useMemo(
    () => DATA.findIndex((d) => d.level === "quietest"),
    [],
  );

  return (
    <div className="w-full bg-white rounded-3xl shadow-[0_20px_60px_-25px_rgba(15,23,42,0.25)] ring-1 ring-slate-200/70 p-6 sm:p-8 md:p-10">
        ...
        <AnimatePresence mode="wait">
          {active !== null && (
            ...
            >
              ...
              <p
                className={`text-base font-semibold ${LEVEL_TEXT[DATA[active].level]}`}
              >
      ...
      >
        <div className="grid grid-cols-7 gap-2 sm:gap-3 md:gap-5 h-[300px] sm:h-[340px] md:h-[360px] items-end pt-8">
          {DATA.map((d, i) => {
            ...
            return (
              ...
              >
                {(d.level === "busiest" || d.level === "quietest") && (
                  ...
                  >
                    <div
                      className="px-2.5 py-1 rounded-full text-[11px] font-bold shadow-sm whitespace-nowrap"
                      style={{
                ...
                <AnimatePresence>
                  {isActive && !isClosed && (
                    <motion.div
                      ...
                      style={{ bottom: `calc(${heightPct}% + 36px)` }}
                    >
