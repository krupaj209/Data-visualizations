...
- **HourlyHeatmapChart** — accepts `compact` and hides the optional `highlight_cards` "when to go and why" strip below the legend. Heatmap grid + Quiet→Crowded legend + best-window pill remain. The cards themselves are an optional `hourly_heatmap` spec field (`{kind, headline, detail}` × ≤6, kinds: `quietest_hours`, `best_photography`, `best_weather`, `fastest_entry`, `best_evening`, `best_off_season`) — curated-only today; AI-generated heatmaps may omit the field. Reference deck: the Colosseum `hourly-crowd-pattern` chart in `lib/curated-seeds/src/data/colosseum.ts`.
